...
def read_data():
    ...
    y_train = pd.read_csv(
        'F:/Eguan/第二届是算法大赛/kpi_train.csv',
        parse_dates=["date"]
...
def submission(fitted):
    df = pd.read_csv('F:/Eguan/第二届是算法大赛/kpi_train.csv', index_col=['date'])
    df = df.iloc[-14:]
    df.index = df.index[:] + 7
    ...
    df.to_csv('F:/Eguan/第二届是算法大赛/submission.csv')
    pass
